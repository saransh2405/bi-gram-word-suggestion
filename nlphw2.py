...
def removeSpecialCharacter(stringToProcess):
	stringToProcess = stringToProcess.replace("'"," ").replace('"'," ").replace('-'," ").replace("`"," ").replace(","," ").lower()
...
stringToProcess = getStringFromFile()
...
wordList,totalWords = getEachWordFrequency(stringToProcess)
twoWordList = getGivenPreviousWordNewWordFrequency(stringToProcess)
